-- EW-SIGINT Script 2.35.07 (Fixed: Pod status update, ELINT naval bands/time, COMINT reports; COMINT now detects all coalitions)
...
-- Version Progression:
-- 2.35.07: Fixed pod status not updating in JAMMER status (refreshed menu); Fixed ELINT unknown band for naval units and strange time format; Fixed COMINT not creating reports/markpoints; Updated COMINT to detect all coalitions.
...
if not mist then
    env.error("EW-SIGINT Script 2.35.07: MIST not loaded. Please load MIST in mission.")
    return
end
...
local POD_TYPES = {
    ...
    ["AN/ALQ-249"] = {power = 1.3, max_range = 60000, name = "AN/ALQ-249 (NGJ Mid-Band)", heat_rate = 0.15, cooldown_rate = 0.8, elint_range = 400000, elint_success_rate = 0.95, comint_capable = true, comint_range = 350000},
    ["AN/APG-81"] = {power = 1.0, max_range = 50000, name = "AN/APG-81 (AESA)", heat_rate = 0.1, cooldown_rate = 1.0, elint_range = 250000, elint_success_rate = 0.92, comint_capable = true, comint_range = 200000},
    ...
    ["2xALQ-249"] = {power = 1.5, max_range = 70000, name = "2x AN/ALQ-249 (Dual NGJ)", heat_rate = 0.17, cooldown_rate = 0.8, elint_range = 450000, elint_success_rate = 0.96, comint_capable = true, comint_range = 400000},
    ...
    ["KLJ-7A"] = {power = 1.0, max_range = 50000, name = "KLJ-7A (AESA)", heat_rate = 0.1, cooldown_rate = 1.0, elint_range = 180000, elint_success_rate = 0.85, comint_capable = true, comint_range = 150000},
    ["AN/APG-77"] = {power = 1.0, max_range = 50000, name = "AN/APG-77 (AESA)", heat_rate = 0.1, cooldown_rate = 1.0, elint_range = 400000, elint_success_rate = 0.95, comint_capable = true, comint_range = 300000},
    ...
    ["ALQ99_NGJ_MB"] = {power = 1.3, max_range = 60000, name = "ALQ99_NGJ_MB", heat_rate = 0.15, cooldown_rate = 0.8, elint_range = 400000, elint_success_rate = 0.95, comint_capable = true, comint_range = 350000},
    ...
    ["Khibiny_L-175V"] = {power = 1.1, max_range = 50000, name = "Khibiny (L-175V) ECM Pod", heat_rate = 0.18, cooldown_rate = 0.6, elint_range = 300000, elint_success_rate = 0.88, comint_capable = true, comint_range = 250000},
    ...
    ["GlobalEye_AESA"] = {power = 1.4, max_range = 65000, name = "GlobalEye AESA System (Swedish/European)", heat_rate = 0.14, cooldown_rate = 0.85, elint_range = 500000, elint_success_rate = 0.96, comint_capable = true, comint_range = 450000},
...
["N036_Byelka"] = {power = 1.4, max_range = 200000, name = "N036 Byelka AESA Radar (Russian)", heat_rate = 0.14, cooldown_rate = 0.85, elint_range = 400000, elint_success_rate = 0.95, comint_capable = true, comint_range = 300000},
...
["P-3C Orion"] = {"AN/ALQ-78"},
    ...
    ["Su-35s"] = {"Khibiny_L-175V", "SAP518_Regatta"},
...
function updateRadarLists()
    ...
    -- env.info("EW-SIGINT Script 2.35.07: Radar lists updated - All: " .. #radarList .. ", Red enemy: " .. #enemyRadarLists[1] .. ", Blue enemy: " .. #enemyRadarLists[2]) -- Commented
...
function startEWjamming(jammer)
    ...
    if not jammerUnit or not jammerUnit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: startEWjamming failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
...
function stopEWjamming(jammer)
    ...
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: stopEWjamming failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    offensiveSwitch[jammer] = nil
    updateMenu(jammer)
...
function setPodType(jammer, pod_type)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setPodType failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    if POD_TYPES[pod_type] then
        jammerPods[jammer] = pod_type
        -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " set to pod: " .. pod_type) -- Commented
...
function setComboPod(jammer, pod_type)
    ...
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setComboPod failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    if POD_TYPES[pod_type] then
...
function setJammingMode(jammer, mode)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setJammingMode failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    jammerModes[jammer] = mode
    -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " offensive mode set to: " .. mode) -- Commented
...
function setDefensiveJammingMode(jammer, mode)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setDefensiveJammingMode failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    defensiveJammerModes[jammer] = mode
    -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " defensive mode set to: " .. mode) -- Commented
...
function setDirectionalSector(jammer, sector)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setDirectionalSector failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    jammerModes[jammer] = "directional"
    jammerSectors[jammer] = sector
    -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " offensive directional sector set to: " .. sector) -- Commented
...
function setDefensiveDirectionalSector(jammer, sector)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setDefensiveDirectionalSector failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    defensiveJammerModes[jammer] = "directional"
    defensiveJammerSectors[jammer] = sector
    -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " defensive directional sector set to: " .. sector) -- Commented
...
function setSpotTarget(jammer, band)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setSpotTarget failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    jammerModes[jammer] = "spot"
    jammerTargets[jammer] = band
    -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " offensive spot target set to: " .. band .. "-Band") -- Commented
...
function setDefensiveSpotTarget(jammer, band)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: setDefensiveSpotTarget failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _groupID = unit:getGroup():getID()
    defensiveJammerModes[jammer] = "spot"
    defensiveJammerTargets[jammer] = band
    -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " defensive spot target set to: " .. band .. "-Band") -- Commented
...
function startELINT(jammer)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: startELINT failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
...
function stopELINT(jammer)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: stopELINT failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    elintSwitch[jammer] = nil
...
function updateMenu(jammer)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: updateMenu failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local _group = unit:getGroup()
    if not _group then
        -- env.info("EW-SIGINT Script 2.35.07: updateMenu failed for " .. jammer .. ": Group not found") -- Commented
        return
    end
    ...
    local cooldown_duration = (aircraft_type == "EA-18G" or aircraft_type == "VSN_F35A" or aircraft_type == "VSN_F35B" or aircraft_type == "VSN_F35C" or aircraft_type == "JF-17" or aircraft_type == "F-22A") and 60 or 120
    local pod_type = jammerPods[jammer]
    -- env.info("EW-SIGINT Script 2.35.07: Creating menu for " .. jammer .. ", aircraft: " .. aircraft_type .. ", groupID: " .. _groupID) -- Commented
    ...
    -- env.info("EW-SIGINT Script 2.35.07: Menu created for " .. jammer .. ", groupID: " .. _groupID) -- Commented
end

function createmenu(jammer)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: createmenu failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    updateMenu(jammer)
end
...
function startDjamming(jammer)
    local jammerUnit = Unit.getByName(jammer)
    if not jammerUnit or not jammerUnit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: startDjamming failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local aircraft_type = jammerUnit:getTypeName()
    local _groupID = jammerUnit:getGroup():getID()
...
function stopDjamming(jammer)
    local unit = Unit.getByName(jammer)
    if not unit or not unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: stopDjamming failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    switch[jammer] = nil
    updateMenu(jammer)
...
function assignPodType(jammer)
    ...
    if not _unit or not _unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: assignPodType failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
    local aircraft_type = _unit:getTypeName()
    local _group = _unit:getGroup()
    if not _group then
        -- env.info("EW-SIGINT Script 2.35.07: assignPodType failed for " .. jammer .. ": Group not found") -- Commented
        return
    end
    ...
    -- env.info("EW-SIGINT Script 2.35.07: Jammer " .. jammer .. " detected as " .. aircraft_type .. ", assigned pod: " .. (pod_type or "none") .. ", ELINT capable: " .. tostring(elintCapable[jammer])) -- Commented
    createmenu(jammer)
end
function EWJamming(jammer)
    local _unit = Unit.getByName(jammer)
    if not _unit or not _unit:isExist() then
        -- env.info("EW-SIGINT Script 2.35.07: EWJamming failed for " .. jammer .. ": Unit not found or does not exist") -- Commented
        return
    end
                ...
                for _, unit in ipairs(group:getUnits()) do
                    ...
                    if not initializedUnits[unitName] then
                        ...
                        if isAlways or hasEW then
                            ...
                            EWJamming(unitName)
                            -- env.info("EW-SIGINT Script 2.35.07: Auto-detected EW unit: " .. unitName) -- Commented
                        end
                    end
                end
            end
